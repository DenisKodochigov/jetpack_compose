...
class DataSourceLocation: PagingSource<Int, LocationAPI>() {
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, LocationAPI> {
        return try {
            val nextPage = params.key ?: 1
            val dataResponse = RetrofitAPI.getDataFormApiPagingLocation(nextPage)
    ...
    override fun getRefreshKey(state: PagingState<Int, LocationAPI>) = 1
}